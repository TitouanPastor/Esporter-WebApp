                                                    ...
                                                    if (!$sameMail3) {
                                                        if (!$samePseudo3) {
                                                            if (!$sameMail4) {
                                                                if (!$samePseudo4) {
                                                                    try {
                                                                        // Récupération de l'ID du dernier utilisateur créer
                                                                        $id = $ecurieModel->getIdEcurieByMail($_SESSION['username']);
                                                                        //Création de l'équipe 
                                                                        $equipeModel->addEquipe(htmlspecialchars($nomEquipe),htmlspecialchars($mdpEquipe),htmlspecialchars($emailEquipe),0,htmlspecialchars($jeuEquipe),$id);
                                                                        ...
                                                                        header('Location: liste-equipes-controller.php?createEquipe=success');
                                                                    } catch (Exception $e) {
...
$buffer = str_replace("##GetDonnee##", "nom=".$nomEquipe."&jeu=".$jeuEquipe."&email=".$emailEquipe."&mdp=".$mdpEquipe, $buffer);
echo $buffer;
